...
def extract_audio_from_uploaded_video(uploaded_video):
    ...
    try:
        ...
        # Extract the audio
        audio = video.audio
        assert audio is not None, "Failed to extract audio from the video"
        return audio
...
def transcribe_video(video_file):
    ...
    with open(audio_file_path, "rb") as audio_file:
        response = client.audio.transcriptions.create(
            file=audio_file, model="whisper-1"  # Specify the model name if required
        )
...
def generate_quiz(text, n_questions, difficulty):
    ...
    response = client.chat.completions.create(
        model="gpt-4o",
        messages=prompts.generate_quizbot_prompts(text, n_questions, difficulty),  # type: ignore
        temperature=0.7,
    )
